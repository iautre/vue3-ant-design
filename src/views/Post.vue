<template>
  <div class="post_content">
    ...
    <div class="icons-list">
      <a-icon type="edit" />
      {{common.formatDate(postData.created)}}
      <template v-if="postData.thumb && postData.thumb.gps">
        <a-icon type="environment" />
        {{ postData.thumb.gps }}
      </template>
      <a-icon type="bars" />
      <template v-for="item in postData.categories">{{item.name}}</template>
      <a-icon type="message" />
      {{postData.commentsNum}}
    </div>
    <div class="content-text" v-html="common.markdown(postData.text,postData.isMarkdown)" />
    <div v-if="postData.tag && postData.tag.length > 0">
      <a-icon type="tag" />:
